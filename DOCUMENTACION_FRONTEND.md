...
2. **Supabase "Web Lock Deadlock":** Supabase usa `navigator.locks` subyacentes. En algunos navegadores, al **recargar** la página el lock del token queda atascado y `getSession()` nunca resuelve → la app se congela en **"Verificando sesión..."**. Se resolvió de raíz **reemplazando `navigator.locks` por un lock en memoria** (`inMemoryLock` en `lib/supabase.ts`, opción `auth.lock`), y como red de seguridad el `AuthContext` resuelve la carga en cuanto conoce la sesión (el perfil se trae aparte) con un **timeout de 5 s** que evita cualquier bloqueo indefinido. En el login además se delegan las transiciones con React Router (sin hard-redirects tras `signInWithPassword`).
