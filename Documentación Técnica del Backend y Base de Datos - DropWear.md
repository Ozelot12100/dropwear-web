...
* **Método de pago en ventas + Corte de Caja:** columna `inventory_items.payment_method` (enum `payment_method` = efectivo/transferencia/tarjeta; se llena solo al vender). Tabla nueva **`cash_cuts`** para el arqueo diario: `cut_date`, `opening_float` (fondo), `sales_cash` (ventas efectivo del día, snapshot), `expected_cash`, `counted_cash`, `difference`, `notes`, `created_by`. Índice por `cut_date`. Autor sellado por `stamp_cash_cut_actor`. RLS abajo.
