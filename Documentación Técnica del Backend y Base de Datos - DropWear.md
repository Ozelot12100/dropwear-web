...
* **RPCs atómicas (Fase 2)** — reemplazan el CRUD multi-paso del cliente por transacciones únicas con `FOR UPDATE` (eliminan la doble venta y el estado inconsistente). `SECURITY INVOKER` (respetan RLS). El actor se sella en el servidor vía triggers (`stamp_log_actor`, `stamp_item_actor`), nunca desde el cliente:
  * `change_item_status(p_item_id, p_new_status, p_price_sold, p_notes, p_reserved_for, p_reserved_contact, p_reserved_until, p_reserved_deposit, p_payment_method)` — cambia estado + inserta el log en una transacción. Exige precio si es venta y nombre de cliente si es apartado. En venta captura el **método de pago** (efectivo por defecto) en `inventory_items.payment_method` y lo antepone en las notas del log.
...
* **Método de pago en ventas + Corte de Caja:** columna `inventory_items.payment_method` (enum `payment_method` = efectivo/transferencia/tarjeta; se llena solo al vender). Tabla nueva **`cash_cuts`** para el arqueo diario: `cut_date`, `opening_float` (fondo), `sales_cash` (ventas efectivo del día, snapshot), `cash_out` (salidas de efectivo: gastos en efectivo/retiros), `expected_cash` (= fondo + ventas efectivo − salidas), `counted_cash`, `difference`, `notes`, `created_by`. Índice por `cut_date`. Autor sellado por `stamp_cash_cut_actor`. RLS abajo.
...
* **Escritura por rol:**
  ...
  * `cash_cuts` (Corte de Caja): igual que `expenses` — **leer** `contador`/`socio`/`superadmin`; **insertar/borrar** solo `socio`/`superadmin`. Actor sellado por `stamp_cash_cut_actor`. Validado E2E (financiero inserta con autor sellado; no financiero recibe RLS 42501).
